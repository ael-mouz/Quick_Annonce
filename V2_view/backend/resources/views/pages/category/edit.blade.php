...
@section('content')
    <div class="row" id="register">
        <form action="{{ route('update_category', $category->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <input type="text" name="sub_cat" class="mb-1 mt-2 form-control" value="{{ $category->sub_cat }}"
                placeholder="Sub categoreis" autocomplete="off" required>
            <textarea name="description" class="mb-1 form-control" placeholder="Description" autocomplete="off" required>{{ $category->description }}</textarea>
